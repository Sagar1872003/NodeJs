...
const UserViewProfile = () => {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                let res = await fetch(`http://localhost:8080/user/userprofile/${id}`, {
                    method: 'GET',
                    headers: {
                    ...
                    {user ? (
                        <div className="card p-4 shadow">
                            ...
                            <div className="mt-4 text-center">
                                <button className="btn btn-primary" onClick={() => navigate(`/user/updateprofile/${user._id}`)}>Update Profile</button>
                            </div>
                        </div>
